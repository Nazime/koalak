...
def get_modules_from_distribution(distribution: PathDistribution, root_package_name)-> list[str]:
    ...
    if is_editable_distribution(distribution):
        editable_filename = [str(e) for e in distribution.files if str(e).startswith("__editable__")]
...
def get_modules_from_distribution_files(distribution: PathDistribution, root_package_name: str) -> list[str]:
    modules = []
